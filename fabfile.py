...
@task()
def deploy(c, skip_checks=False):
    """
    Deploy project.
    """
    check_branch(c)
...
def push_sources(c: Connection, target: Version):
    ...
    if not files.exists(c, os.path.join(target_src_root, ".git")):
        ...
        if files.exists(c, previous_src_root) and files.exists(c, os.path.join(previous_src_root, ".git")):
            ...
            # what we are pushing.
            c.run(f"git clone {previous_src_root} {target_src_root}")
            with c.cd(target_src_root):
                c.run("git checkout master || git checkout -b master")
        else:
            with c.cd(target_src_root):
                c.run("git config --global init.defaultBranch master")
                c.run("git init")
        with c.cd(target_src_root):
            c.run("echo '[receive]' >> .git/config")
            c.run("echo 'denyCurrentBranch = ignore' >> .git/config")

    c.local(f"git push ssh://{c.user}@{c.host}/{target_src_root}")
    with c.cd(target_src_root):
        c.run(f"git reset --hard {target.version}")
    ...
    # Need settings file
    with c.cd(target_src_root):
        c.run("cp cciw/settings_local_example.py cciw/settings_local.py")
...
def push_non_vcs_sources(c, target=None):
    ...
    for src in NON_VCS_SOURCES:
        c.local(f"rsync {src} {c.user}@{c.host}:{target.SRC_ROOT}/{src}")
...
def get_non_vcs_sources(c):
    ...
    target = Version.current()
    for src in NON_VCS_SOURCES:
        c.local(f"rsync {c.user}@{c.host}:{target.SRC_ROOT}/{src} {src}")


def tag_deploy(c: Connection):
    c.local("git tag deploy-production-$(date --iso-8601=seconds | tr ':' '-' | cut -f 1 -d '+')")
    c.local("git push --tags origin")
...
def create_venv(c, target):
    ...
    c.run(f"virtualenv --python={PYTHON_BIN} {venv_root}")
...
def stop_webserver(c):
    ...
    """
    supervisorctl(f"stop {PROJECT_NAME}_uwsgi")
...
def start_webserver(c):
    ...
    """
    supervisorctl(f"start {PROJECT_NAME}_uwsgi")
...
@root_task()
def supervisorctl(c, *commands):
    c.run(f"supervisorctl {' '.join(commands)}")
...
def upload_usermedia(c):
    ...
    target = Version.current()
    c.local(
        f"rsync -z -r --progress {LOCAL_USERMEDIA}/ {PROJECT_USER}@{c.host}:{target.MEDIA_ROOT}",
        capture=False,
    )
    c.run(f"find -L {target.MEDIA_ROOT} -type f -exec chmod ugo+r {{}} ';'")
    c.run(f"find {target.MEDIA_ROOT_SHARED} -type d -exec chmod ugo+rx {{}} ';'")
