...
env.PROJECT_NAME = "cciw"
env.PROJECT_PYTHON_MODULE = "cciw"
...
WEBAPPS_ROOT = f"/home/{env.PROJECT_USER}/webapps"
...
def provision():
    ...
    run(f"mkdir -p /home/{env.PROJECT_USER}/logs")
...
def push_non_vcs_sources(target=None):
    ...
    for s in NON_VCS_SOURCES:
        local(f"rsync {s} {env.PROJECT_USER}@{env.hosts[0]}:{target.SRC_ROOT}/{s}")
...
def get_non_vcs_sources():
    ...
    target = Version.current()
    for s in NON_VCS_SOURCES:
        local(f"rsync {env.PROJECT_USER}@{env.hosts[0]}:{target.SRC_ROOT}/{s} {s}")
...
def rsync_dir(local_dir, dest_dir):
    ...
    local(
        f"rsync -z -r -L --delete --exclude='_build' --exclude='.hg' --exclude='.git' --exclude='.svn' --delete-excluded {local_dir}/ {env.PROJECT_USER}@{env.hosts[0]}:{dest_dir}",
        capture=False,
    )
...
def migrate_upload_db(local_filename):
    ...
    remote_filename = f"/home/{env.PROJECT_USER}/{os.path.basename(local_filename)}"
...
def upload_usermedia():
    ...
    target = Version.current()
    local(
        f"rsync -z -r --progress {LOCAL_USERMEDIA}/ {env.PROJECT_USER}@{env.hosts[0]}:{target.MEDIA_ROOT}",
        capture=False,
    )
...
def download_usermedia():
    target = Version.current()
    local(f"rsync -z -r  {env.PROJECT_USER}@{env.hosts[0]}:{target.MEDIA_ROOT}/ {LOCAL_USERMEDIA}", capture=False)
