...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm.MenuLink.objects.filter(title='Home').update(visible=False)
        orm.MenuLink.objects.filter(title='About CCIW').update(title='About', listorder=150)
        orm.MenuLink.objects.filter(title='CCIW camp sites').update(title='Camp sites')
        orm.MenuLink.objects.filter(title='Leaders and officers').update(title='Officers')
        orm.MenuLink.objects.filter(title='About website').update(visible=False)
        orm.MenuLink.objects.filter(title='Awards').update(visible=False)
        orm.MenuLink.objects.filter(title='Members').update(visible=False)
        orm.MenuLink.objects.filter(title='Sign up').update(visible=False)
        orm.MenuLink.objects.filter(title='Forums and photos').update(title='Forums')


    def backwards(self, orm):
        pass # No point doing this
