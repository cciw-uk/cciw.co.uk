...
urlpatterns = patterns('cciw.apps.cciw.views',
	...
	(r'^camps/(?P<year>\d{4})/(?P<number>\d+)/photos/$', 'camps.photos'),
	(r'^camps/(?P<galleryname>[^/]*)/photos/$', 'camps.oldcampphotos'),
	...
	(r'^news/(?P<topicid>\d+)/$', 'forums.topic', {'title': 'News'}), # TODO - create different template ?
