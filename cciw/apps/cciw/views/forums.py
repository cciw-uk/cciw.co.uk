...
def topic_breadcrumb(forum, topic):
	if len(topic.subject) > 30:
		subject = '<span title="' + escape(topic.subject) + '">' +  topic.subject[0:27] + "...</span>"
	else:
		subject = topic.subject
		
	return ['<a href="' + forum.get_absolute_url() + '">Topics</a>', subject]
...
def topic(request, title = None, extra_context = None, topic = None, template_name = 'forums/topic', topicid = 0,
