...
def _bogofilter_command(args: list[str]) -> list[str | PosixPath]:
    return ["bogofilter", "-d", settings.BOGOFILTER_DIR] + args


def make_email_msg(from_email: str, subject: str, body: str, extra_headers: dict | None = None) -> bytes:
...
def get_bogofilter_classification(msg_bytes: bytes) -> tuple[BogofilterStatus, float | None]:
