...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Need to delete all the application forms that violate this new
        # constraint.
        # Collect all application forms into dict by (camp id, officer id)
        all_apps = {}

        for a in orm.Application.objects.all():
            all_apps.setdefault((a.camp_id, a.officer_id), []).append(a)

        for (camp_id, officer_id), apps in all_apps.iteritems():
            if len(apps) > 1:
                # Need to decide which to keep
                # Applications that are finished have priority
                # Applications finished later have priority
                # - recognise by date_submitted and PK
                keyfunc = lambda app: (app.finished,
                                       app.date_submitted
                                       if app.date_submitted is not None
                                       else datetime.date(1970,1,1),
                                       app.id)
                apps.sort(key=keyfunc, reverse=True)
                for a in apps[1:]:
                    a.delete()
    ...
    models = {
        ...
        'officers.application': {
            'Meta': {'ordering': "('-camp__year', 'officer__first_name', 'officer__last_name', 'camp__number')", 'unique_together': "(('camp', 'officer'),)", 'object_name': 'Application'},
