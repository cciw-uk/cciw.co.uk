...
from .test_data import BAD_MESSAGE_1
...
def test_handle_role_list():
    role = _setup_role_for_email(
...
def test_handle_public_role_list():
    role = _setup_role_for_email(
...
def test_handle_internationalised_headers():
    role = _setup_role_for_email(
        email="myrole@mailtest.cciw.co.uk",
        allow_emails_from_public=True,
        recipients=[("test1", "test1@example.com")],
...
def test_handle_bad_message_malformed_1():
...
def test_handle_officer_list():
    ...
    assert all(m.from_email == '"Kevin Smith kevin.smith(at)example.com" <noreply@cciw.co.uk>' for m in sent_messages)
...
def test_spam_and_virus_checking():
    role = _setup_role_for_email(
        name="Test",
...
def test_handle_mail_exception():
    """
...
def test_handle_partial_sending_failure():
    """
...
def test_handle_mail_permission_denied():
...
def test_handle_mail_permission_denied_for_unknown():
